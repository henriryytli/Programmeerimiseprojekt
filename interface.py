...
akna_laius = app.winfo_reqwidth()
akna_kõrgus = app.winfo_reqheight()

# TAUSTAPILT

pilt = Image.open("TAUST.png")
pilt = pilt.resize((akna_laius,akna_kõrgus), Image.LANCZOS)
pilt = ImageTk.PhotoImage(pilt)
canvas = Canvas(app, width=akna_laius, height=akna_kõrgus)
canvas.pack(fill=BOTH, expand=True)
canvas.create_image(0, 0, image=pilt, anchor='nw')
